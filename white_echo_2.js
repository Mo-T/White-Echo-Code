...
function updateRoom() {
	...
	var room = this.dataset.name; //selected room
...
function updateBooking(dailyPrice,room) {
	var totalCost = 0;
	...
	totalCost += Number(numberNights) + breakfastTotal + addCost;
	...
	checkInputs(numberNights, checkedAddItems, totalCost, room);
}
...
function checkInputs(numberNights, checkedAddItems, totalCost, room) {
	...
	if (document.getElementById("checkInDate").validity.valueMissing || document.getElementById("checkInDate").validity.rangeUnderFlow) {
		document.getElementById("errorMessage-1").innerHTML = "Please enter in a valid check in date thats is not a past date";
	...
	pushData(firstName, lastName, cellphoneNumber, emailAddress, room, totalCost, checkedAddItems, numberNights, checkedAddItems);
}

function pushData(firstName, lastName, cellphoneNumber, emailAddress, room, totalCost, checkedAddItems, numberNights) {
